...
- Sleep category intervals are not persisted as daily duration. Asleep-stage intervals are grouped with a documented 90-minute gap and unioned to prevent overlap double counting. Today selects the primary Apple session inside the server's UTC overnight window (previous day 18:00 through current day 14:00): meaningful sessions (>=90 min), then duration, proximity to 07:00, and stable tie-breakers. Naps remain stored as secondary sessions.
